...
/*   Updated: 2025/02/27 04:11:35 by mochamsa         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	exec_single_cmd(t_shell *shell, t_command *cmd, t_exec_data *exec)
{
	...
	if (is_builtin(cmd->args[0]) != NOT_BUILTIN)
	{
		if (cmd->next)
		{
			if (cmd->pid == 0)
			{
				...
				(free_env_array(&exec->env_arr),	free_env(shell->env));
	...
	if (!cmd_path)
		return (handle_cmd_not_found(cmd->args[0]), clean_heredoc_f(shell->here_docs,cmd->pid), 127);
